...
ALTER TABLE CART ADD COLUMN IF NOT EXISTS `VIEW_YN` CHAR(1) NULL  DEFAULT 'Y' COMMENT '사용자 보기 여부(바로구매이면 N으로)'
;

ALTER TABLE ORDR_DTL ADD COLUMN IF NOT EXISTS `GDS_OPTN_NO` INT NULL DEFAULT NULL COMMENT '상품 옵션 번호'
;
ALTER TABLE ORDR_DTL ADD COLUMN IF NOT EXISTS `DLVY_GROUP_YN` CHAR(1) NULL DEFAULT NULL COMMENT '묶음배송'
;
ALTER TABLE ORDR_DTL ADD COLUMN IF NOT EXISTS `DLVY_CT_CND` CHAR(1) NULL DEFAULT NULL COMMENT '배송_비용_조건(DLVY_CT_TY에서 조건부 무료이면 무료조건 금액, 구매수량별 부과 이면 갯수)'
;
...
CREATE TABLE `ENTRPS_DLVYGRP` (
	...
	`ENTRPS_DLVYGRP_NM` VARCHAR(20) NOT NULL COMMENT '업체별 묶음배송 명' COLLATE 'utf8mb4_unicode_ci',
