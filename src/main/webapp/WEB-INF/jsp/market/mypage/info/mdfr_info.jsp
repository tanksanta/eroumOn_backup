...
<main id="container">
	...
	<jsp:include page="../../layout/page_sidenav.jsp" />
		...
		<div id="page-content">
	        <ul class="tabs">
	            <li><a href="${_marketPath}/mypage/info/form" class="tabs-link active"><strong>회원정보</strong> 수정</a></li>
	            <li><a href="${_marketPath}/mypage/info/pswd" class="tabs-link"><strong>비밀번호</strong> 변경</a></li>
	        </ul>

            <div class="mypage-myinfo-modify mt-11 md:mt-15">

				<form:form action="${_marketPath}/mypage/info/infoAction" id="frmReg" name="frmReg" method="post" modelAttribute="mbrVO" enctype="multipart/form-data" class="member-join-content">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                	...
                                	<p class="py-1.5 text-sm">전화번호를 입력해주세요</p>
                               	</td>
                            </tr>
                            <tr>
                                <th scope="row">
                                    <p><label for="eml">이메일<sup class="text-danger text-base md:text-lg">*</sup></label></p>
                                </th>
                                <td><form:input class="form-control w-full max-w-73" path="eml" maxlength="50"/></td>
                            </tr>
                            <tr>
                                <th scope="row">
                                    <p><label for="addr">주소<sup class="text-danger text-base md:text-lg">*</sup></label></p>
                                </th>
                                <td>
                    ...
                    <table class="table-detail">
                        ...
                        <tbody>
                            ...
                            <tr class="wrapNm">
                                <th scope="row"><p><label for="recipter">수급자 성명</label></p></th>
                                <td><input type="text" class="form-control w-full" id="recipter" name="recipter" maxlength="50" value="${mbrVO.mbrNm}"></td>
                            </tr>
                            <tr class="wrapNo">
                                <th scope="row"><p style="padding-left: 0;"><label for="rcperRcognNo">요양인정번호</label></p></th>
                                <td>
                            ...
                            <!-- <tr class="wrapGrade">
                                <th scope="row"><p><label for="rcognGrad">등급선택</label></p></th>
                                <td>
									<select id="rcognGrad" name="rcognGrad" class="form-control w-full">
										<option value="" class="noneGrade">등급 선택</option>
										<option value="0" <c:if test="${mbrVO.recipterInfo.rcognGrad eq '0'}">selected="selected"</c:if>>등급 외</option>
										<option value="1" <c:if test="${mbrVO.recipterInfo.rcognGrad eq '1'}">selected="selected"</c:if>>1 등급</option>
										<option value="2" <c:if test="${mbrVO.recipterInfo.rcognGrad eq '2'}">selected="selected"</c:if>>2 등급</option>
										<option value="3" <c:if test="${mbrVO.recipterInfo.rcognGrad eq '3'}">selected="selected"</c:if>>3 등급</option>
										<option value="4" <c:if test="${mbrVO.recipterInfo.rcognGrad eq '4'}">selected="selected"</c:if>>4 등급</option>
										<option value="5" <c:if test="${mbrVO.recipterInfo.rcognGrad eq '5'}">selected="selected"</c:if>>5 등급</option>
										<option value="6" <c:if test="${mbrVO.recipterInfo.rcognGrad eq '6'}">selected="selected"</c:if>>6 등급</option>
									</select>
                                </td>
                            </tr>
                            <tr class="wrapSelf">
                                <th scope="row"><p style="padding-left: 0;"><label for="join-item2-4">본인부담금율</label></p></th>
                                <td>
                                	<select name="selfBndRt" id="selfBndRt" class="form-control w-full">
										<option value="" class="noneGrade">본인부담금율 선택</option>
										<option value="15, 일반대상자" <c:if test="${mbrVO.recipterInfo.selfBndRt eq '15' && mbrVO.recipterInfo.selfBndMemo eq '일반대상자'}">selected="selected"</c:if>>일반대상자(15%)</option>
										<option value="9, 감경대상자" <c:if test="${mbrVO.recipterInfo.selfBndRt eq '9' && mbrVO.recipterInfo.selfBndMemo eq '감경대상자'}">selected="selected"</c:if>>감경대상자(9%)</option>
										<option value="6, 감경대상자" <c:if test="${mbrVO.recipterInfo.selfBndRt eq '6' && mbrVO.recipterInfo.selfBndMemo eq '감경대상자'}">selected="selected"</c:if>>감경대상자(6%)</option>
										<option value="6, 의료급여수급권자" <c:if test="${mbrVO.recipterInfo.selfBndRt eq '6' && mbrVO.recipterInfo.selfBndMemo eq '의료급여수급권자'}">selected="selected"</c:if>>의료급여수급권자(6%)</option>
										<option value="0, 기초생활수급권자" <c:if test="${mbrVO.recipterInfo.selfBndRt eq '0' && mbrVO.recipterInfo.selfBndMemo eq '기초생활수급권자'}">selected="selected"</c:if>>기초생활수급권자(0%)</option>
									</select>
                                </td>
                            </tr> -->
                        </tbody>
					...
					<div class="content-recipient mt-9.5" id="wrapInfo">
							...
							<div class="group2">
								...
								<dl class="date">
									<dt>인정유효기간</dt>
									<dd>
										<p id="searchBgngRcgt"><fmt:formatDate value="${mbrVO.recipterInfo.vldBgngYmd}" pattern="yyyy-MM-dd" /></p>
										<p id="searchEndRcgt">~&nbsp;<fmt:formatDate value="${mbrVO.recipterInfo.vldEndYmd}" pattern="yyyy-MM-dd" /></p>
									</dd>
								</dl>
								<dl class="date">
									<dt>적용기간</dt>
									<dd>
										<p id="searchBgngApdt"><fmt:formatDate value="${mbrVO.recipterInfo.aplcnBgngYmd}" pattern="yyyy-MM-dd" /></p>
										<p id="searchEndApdt">~&nbsp;<fmt:formatDate value="${mbrVO.recipterInfo.aplcnEndYmd}" pattern="yyyy-MM-dd" /></p>
									</dd>
								</dl>
							</div>
							<dl class="price1">
                    ...
                    <table class="table-detail">
                        ...
                        <tbody>
							...
							<%--
							<tr>
                                <th scope="row"><p><label for="rcmdtnId">추천인 아이디</label></p></th>
								<td>
									<form:input class="form-control w263" path="rcmdtnId" maxlength="15"/>
                                    <p class="text-sm mt-2.5">추천인에게 포인트가 제공됩니다.</p>
                                </td>
							</tr>
							--%>
                            <tr>
                                <th scope="row"><p><label for="telno">전화번호</label></p></th>
                                <td>
                                    <form:input class="form-control w-full max-w-73" path="telno" maxlength="13"/>
                                    <p class="text-sm mt-2.5">숫자만 입력해주세요</p>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><p><label for="join-item3-3">개인정보<br class="xs:hidden"> 유효기간</label></p></th>
                                <td>
									<div class="form-check-group w-full max-w-73">
                            ...
                            <tr>
                                <th scope="row"><p><label for="join-item3-4">정보수신</label></p></th>
                                <td>
	...
	//수급자 정보 조회
		...
		.done(function(json) {
			if(json.result){
				...
				$("#selfBndRt").val(Number(json.infoMap.QLF_TYPE));
