	...
	<main id="container">
        ...
        <div id="page-container">
            <div id="page-content">
                ...
                <div class="mb-9 space-y-6 lg:mb-12 md:space-y-7.5">
					...
					<div class="order-checkitem">
                        <div class="form-check">
                            ...
                            <input type="hidden" name="R_gdsPc" value="${cart.ordrPc}" />
                            <input type="hidden" name="R_dlvyPc" value="${cart.gdsInfo.dlvyAditAmt + cart.gdsInfo.dlvyBassAmt}" />
	                                        ...
	                                        <div class="order-item-base">
	                                            <p class="code">
	                                                <span class="label-primary">
	                                                    <span>${gdsTyCode[cart.gdsInfo.gdsTy]}</span>
	                                                    <i></i>
	                                                </span>
						...
						<c:set var="sumOrdrPc" value="${sumOrdrPc + cart.ordrPc}" />
	                                        <div class="order-item-add">
	                                            ...
	                                            <div class="name">
		                                            ...
		                                            <p>수량 ${cart.ordrQy}개 (+<fmt:formatNumber value="${cart.ordrPc}" pattern="###,###" />원)</p>
	                                            </div>
	                                        </div>
						</c:if>
						...
						<c:if test="${rResultList[status.index+1].ordrOptnTy eq 'BASE' || status.last}">
	                                    ...
	                                    <div class="order-item-info">
	                                        <div class="payment">
	                                        	<c:if test="${cart.cartTy eq 'R'}"><%-- 급여구매일 경우만 경우만 멤버스(사업소) 있음 --%>
                ...
                <div class="order-amount mt-8 lg:mt-10">
                    <div class="container">
                        <div class="amount-section">
                            <dl>
                                <dt>상품금액</dt>
                                <dd><strong class="R_totalGdsPc"><fmt:formatNumber value="${totalOrdrPc}" pattern="###,###" /></strong> 원</dd>
                            </dl>
                            <i class="is-plus">+</i>
                            <dl>
                                <dt>배송비</dt>
                                <dd><strong class="R_totalDlvyPc"><fmt:formatNumber value="${totalDlvyBassAmt}" pattern="###,###" /></strong> 원</dd>
                            </dl>
                        </div>
                        <div class="amount-section">
                            <i class="is-equal">=</i>
                            <dl class="text-danger">
                                <dt>총 결제예정금액</dt>
                                <dd><strong class="R_totalStlmPc"><fmt:formatNumber value="${totalOrdrPc + totalDlvyBassAmt}" pattern="###,###" /></strong> 원</dd>
                            </dl>
                        </div>
                    </div>
                </div>
                ...
                <div class="order-amount mt-8 lg:mt-10">
                    <div class="container">
                        <div class="amount-section">
                            <dl>
                                <dt>상품금액</dt>
                                <dd><strong class="L_totalGdsPc"><fmt:formatNumber value="${totalOrdrPc}" pattern="###,###" /></strong> 원</dd>
                            </dl>
                            <i class="is-plus">+</i>
                            <dl>
                                <dt>배송비</dt>
                                <dd><strong class="L_totalDlvyPc"><fmt:formatNumber value="${totalDlvyBassAmt}" pattern="###,###" /></strong> 원</dd>
                            </dl>
                        </div>
                        <div class="amount-section">
                            <i class="is-equal">=</i>
                            <dl class="text-danger">
                                <dt>총 결제예정금액</dt>
                                <dd><strong class="L_totalStlmPc"><fmt:formatNumber value="${totalOrdrPc + totalDlvyBassAmt}" pattern="###,###" /></strong> 원</dd>
                            </dl>
                        </div>
                    </div>
                </div>
                ...
                <h3 class="text-title mt-10 md:mt-14 lg:mt-19"><strong class="text-danger">꼭</strong> 확인해 주세요</h3>
		...
		<form id="frmCart" name="frmCart" method="post" enctype="multipart/form-data">
			<input type="hidden" id="cartGrpNos" name="cartGrpNos" value="">
			<input type="hidden" id="cartTy" name="cartTy" value="">
		</form>
    </main>
    ...
    <script>
   	$(function(){
		...
		$(".f_checkAll").on("click", function(){
			...
			$(":checkbox[name=cartGrpNo].cart_ty_"+cartTy).prop("checked",isChecked);
			$(":checkbox[name=cartGrpNo].cart_ty_"+cartTy).parents(".order-checkitem").find(".order-product").removeClass("is-active");
		...
		let totalsGdsPc = 0;
   		let totalsDlvyPc = 0;
   		let totalsStlmPc = 0;
		$(".cart_ty_R").on("click", function(){
			let isChecked = $(this).is(":checked");
			let gdsPcs = $(this).siblings("input[name='R_gdsPc']").val();
			let dlvyPcs = $(this).siblings("input[name='R_dlvyPc']").val();
			let stlmPcs = $(this).siblings("input[name='R_stlmPc']").val();
			$(this).prop("checked",isChecked);
			if(isChecked){
				$(this).parents(".order-checkitem").find(".order-product").addClass("is-active");
				totalsGdsPc = Number(totalsGdsPc) + Number(gdsPcs);
				totalsDlvyPc = Number(totalsDlvyPc) + Number(dlvyPcs);
				totalsStlmPc = Number(totalsStlmPc) + Number(stlmPcs);
			}else{
			...
			if($(":checkbox[name='cartGrpNo'].cart_ty_R:checked").length < 1){
				...
				let orisDlvyPc = 0;
				let orisStlmPc = 0;
				$("input[name='R_gdsPc']").each(function(){
					orisGdsPc  = Number(orisGdsPc) + Number($(this).val());
				});
				$("input[name='R_dlvyPc']").each(function(){
					orisDlvyPc  = Number(orisDlvyPc) + Number($(this).val());
				});
				$("input[name='R_stlmPc']").each(function(){
					orisStlmPc  = Number(orisStlmPc) + Number($(this).val());
				});
   		...
   		$(".f_deleteSel").on("click", function(){
   			...
   			if(confirm(confirmMsg)) {
				$.ajax({
					type : "post",
					url  : "./removeCart.json",
					data : {cartTy:cartTy, cartGrpNos:cartGrpNos, recipterUniqueId:"${_mbrSession.prtcrRecipterInfo.uniqueId}"},
					dataType : 'json'
				})
				.done(function(data) {
					if(data.result){
		...
		$(".f_ordr").on("click", function(){
			...
			} else if(cartTy == "R" && $(":checkbox[name=cartGrpNo]:checked.cart_ty_R[data-cart-ty='L']").length > 1){
